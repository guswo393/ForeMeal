...
public interface PantryItemRepository extends JpaRepository<PantryItem, Long>{
    ...
    @Query("select i from PantryItem i left join fetch i.foodMaster where i.userId = :userId")
    List<PantryItem> findAllByUserIdWithFoodMaster(@Param("userId") Long userId);

    // 2. 특정 사진 스캔(scanId) 이력을 통해 냉장고에 대량 등록된 재료 목록 조회
    @Query("select i from PantryItem i where i.pantryScan.scanId = :scanId")
    List<PantryItem> findByPantryScanId(@Param("scanId") Long scanId);

    // 3. 유통기한이 지난 냉장고 재료들만 골라내기 (알림 보낼 때 사용)
    @Query("select i from PantryItem i where i.userId = :userId and i.expirationDate < current_date")
